...
const Signup = () => {
    ...
    return (
        <div className="min-h-screen flex items-center justify-center bg-[#0a0a0a] p-4 font-sans text-gray-200 py-12">
            <div className="absolute bottom-0 left-0 w-72 h-72 bg-indigo-600/10 blur-[100px] rounded-full pointer-events-none"></div>
            ...
            <div className="bg-white/[0.02] border border-white/5 p-8 sm:p-10 rounded-3xl w-full max-w-md relative z-10 shadow-2xl backdrop-blur-md">
                ...
                <p className="text-gray-400 text-sm text-center mb-8 font-medium">Create your account to start bidding or managing auctions.</p>
                ...
                <form onSubmit={handleSubmit} className="space-y-4" noValidate>
                    <div>
                        <label htmlFor="name" className="block text-sm font-semibold text-gray-300 mb-1.5 ml-1">Full Name</label>
                        <input
                            id="name"
                            name="name"
                            type="text"
                            ...
                            className={`w-full bg-black/40 border ${validationErrors.name ? 'border-red-500/50 focus:border-red-400' : 'border-white/10 focus:border-indigo-500'} rounded-xl px-4 py-3.5 text-white focus:outline-none focus:ring-1 ${validationErrors.name ? 'focus:ring-red-400' : 'focus:ring-indigo-500'} transition-all shadow-inner`}
                            placeholder="John Doe"
                        />
                        {validationErrors.name && <p className="text-red-400 text-xs mt-1.5 ml-1 font-medium">{validationErrors.name}</p>}
                    </div>
                    ...
                    <div className="pt-2">
                        <label htmlFor="role" className="block text-sm font-semibold text-gray-300 mb-1.5 ml-1">Role</label>
                        <select
                            id="role"
                            name="role"
                            value={formData.role}
                            onChange={handleChange}
                            className="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-3.5 text-white focus:outline-none focus:ring-1 focus:ring-indigo-500 transition-all shadow-inner appearance-none bg-[url('data:image/svg+xml;charset=US-ASCII,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20width%3D%2224%22%20height%3D%2224%22%20viewBox%3D%220%200%2024%2024%22%20fill%3D%22none%22%20stroke%3D%22%239ca3af%22%20stroke-width%3D%222%22%20stroke-linecap%3D%22round%22%20stroke-linejoin%3D%22round%22%3E%3Cpolyline%20points%3D%226%209%2012%2015%2018%209%22%3E%3C%2Fpolyline%3E%3C%2Fsvg%3E')] bg-no-repeat bg-[position:right_1rem_center] bg-[length:1.2em_1.2em] pr-10"
                        >
                            <option className="bg-gray-900 text-white" value="DEALER">Dealer</option>
                            <option className="bg-gray-900 text-white" value="ADMIN">Admin</option>
                        </select>
                        <p className="text-[11px] text-gray-500 mt-2.5 px-1 font-medium leading-relaxed">
                            <span className="text-indigo-400">Admin</span> can create and manage auctions <span className="opacity-50 mx-1">•</span> <span className="text-emerald-400">Dealers</span> can participate in live bidding
                        </p>
                    </div>
                    ...
                    <button
                        type="submit"
                        disabled={loading}
                        className="w-full bg-indigo-600 hover:bg-indigo-500 text-white font-bold py-3.5 rounded-xl transition-all mt-8 disabled:opacity-70 disabled:cursor-not-allowed shadow-[0_0_20px_rgba(79,70,229,0.3)] hover:shadow-[0_0_30px_rgba(79,70,229,0.4)] active:scale-[0.98] flex items-center justify-center gap-2"
                    >
                        {loading ? (
                            <>
                ...
                <p className="mt-8 text-center text-gray-500 text-sm font-medium">
                    ...
                    <Link to="/login" className="text-indigo-400 hover:text-indigo-300 transition-colors font-bold ml-1">
                        Sign in
                    </Link>
                </p>
            </div>
